...
class PackageStats(Base):
    ...
    def get_all_visits(cls, dataset_id) -> Visits:
        ...
        current_end_of_week: datetime = get_end_of_week(datetime.now())
        start_date: datetime = get_beginning_of_next_week(
            current_end_of_week.replace(hour=0, minute=0, second=0, microsecond=0)
            - timedelta(days=365)
        )
        ...
        while current_end_of_week > start_date:
            ...
            print(f"\nChecking week: {current_start_of_week.date()} - {current_end_of_week.date()}")

            for visit in visits:
                raw_date = visit.get('visit_date', '')
                print(f"Raw visit_date string: {raw_date}")

                try:
                    visit_date: datetime = datetime.strptime(raw_date, "%d-%m-%Y")
                except ValueError:
                    print("INVALID date format!", raw_date)
                    continue

                print(f"Parsed visit_date: {visit_date.date()}")

                if not (current_start_of_week <= visit_date <= current_end_of_week):
                    print(f"Skipping visit_date {visit_date.date()} not in week range")
                    continue

                weekly_download_count += visit.get('downloads', 0)
                weekly_visit_count += visit.get('visits', 0)

            print(f"Week summary -> Visits: {weekly_visit_count}, Downloads: {weekly_download_count}")

            visit_list.append({
                'year': current_end_of_week.year,
                'week': current_end_of_week.isocalendar()[1],
                'visits': weekly_visit_count,
                'downloads': weekly_download_count
            })

            current_end_of_week -= timedelta(weeks=1)
