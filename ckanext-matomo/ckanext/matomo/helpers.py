...
def get_downloads_in_date_range_for_resource(id: str) -> Visits:
    end_of_current_week = get_end_of_week(datetime.now())
    start_date = get_beginning_of_next_week(end_of_current_week.replace(hour=0, minute=0, second=0, microsecond=0)
                                            - timedelta(days=365))
    return ResourceStats.get_downloads_in_date_range_by_id(id, start_date, end_of_current_week)
