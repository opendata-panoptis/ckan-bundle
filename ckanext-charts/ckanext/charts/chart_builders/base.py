...
class BaseChartBuilder(ABC):
    ...
    def __init__(
        ...
        self._convert_timestamps_for_js()
    ...
    def _convert_timestamps_for_js(self):
        """
        Μετατρέπει timestamps από διάφορες μορφές σε readable datetime strings.

        Η μέθοδος χειρίζεται τρεις κύριες περιπτώσεις:
        1. Στήλες με τύπο datetime64[ns]
        2. Στήλες με τύπο int64 που περιέχουν nanosecond timestamps
        3. Στήλες με τύπο object που περιέχουν string representations των timestamps

        Αυτή η μετατροπή είναι απαραίτητη γιατί οι μηχανές γραφημάτων (ECharts, Plotly, κλπ)
        δεν μπορούν να διαβάσουν σωστά τα nanosecond timestamps που παράγει το pandas.
        """

        # Επανάληψη σε όλες τις στήλες του DataFrame
        for col in self.df.columns:

            # ΠΕΡΙΠΤΩΣΗ 1: Στήλες που είναι ήδη datetime
            if self.df[col].dtype == 'datetime64[ns]':
                # Μετατρέπουμε τα datetime objects σε human-readable strings
                # Format: "YYYY-MM-DD HH:MM:SS" (π.χ. "2023-08-01 00:00:00")
                self.df[col] = pd.to_datetime(self.df[col]).dt.strftime('%Y-%m-%d %H:%M:%S')

            # ΠΕΡΙΠΤΩΣΗ 2: Στήλες με ακέραιους αριθμούς (int64) που μπορεί να είναι timestamps
            elif self.df[col].dtype == 'int64':
                try:
                    # Παίρνουμε δείγμα από τις πρώτες 10 μη-κενές τιμές για έλεγχο
                    sample_values = self.df[col].dropna().head(10)

                    # Ελέγχουμε αν έχουμε τουλάχιστον μία τιμή για ανάλυση
                    if len(sample_values) > 0:

                        # Ελέγχουμε αν οι τιμές είναι nanosecond timestamps
                        # Nanosecond timestamps είναι μεγάλοι αριθμοί με 19 ψηφία
                        # Εύρος: 1000000000000000000 έως 9999999999999999999
                        # (αντιστοιχεί περίπου στα έτη 1970-2286)
                        is_nanosecond_timestamp = sample_values.apply(
                            lambda x: isinstance(x, (int, float)) and
                                      1000000000000000000 <= abs(x) <= 9999999999999999999
                        ).any()  # .any() επιστρέφει True αν έστω μία τιμή πληροί τη συνθήκη

                        # Πρόσθετοι έλεγχοι
                        if is_nanosecond_timestamp:
                            try:
                                converted_dates = pd.to_datetime(sample_values)

                                # Έλεγχος 1: Οι ημερομηνίες είναι σε λογικό εύρος; (1970-2100)
                                min_reasonable_date = pd.Timestamp('1970-01-01')
                                max_reasonable_date = pd.Timestamp('2100-01-01')

                                dates_in_reasonable_range = (
                                    (converted_dates >= min_reasonable_date) &
                                    (converted_dates <= max_reasonable_date)
                                ).all()

                                # Έλεγχος 2: Το όνομα της στήλης υποδηλώνει ημερομηνία;
                                column_suggests_datetime = any(keyword in col.lower() for keyword in
                                                               ['date', 'time', 'timestamp', 'created', 'updated',
                                                                'modified',
                                                                'ημερομηνια', 'χρονος', 'ωρα'])

                                # ΜΟΝΟ αν περάσουν ΚΑΙ οι δύο έλεγχοι και μάλλον είμαστε σε nanosecond timestamps
                                if dates_in_reasonable_range and column_suggests_datetime:
                                    # Μετατρέπουμε από nanoseconds σε datetime και μετά σε string
                                    # pd.to_datetime() αναγνωρίζει αυτόματα nanosecond timestamps
                                    self.df[col] = pd.to_datetime(self.df[col]).dt.strftime('%Y-%m-%d %H:%M:%S')

                            except (ValueError, OverflowError):
                                # Αν η μετατροπή σε datetime αποτύχει, δεν είναι timestamp
                                pass

                except (TypeError, ValueError, OverflowError):
                    # Αν οποιαδήποτε από τις παραπάνω λειτουργίες αποτύχει,
                    # αφήνουμε τη στήλη ως έχει χωρίς να διακόπτουμε τη διαδικασία
                    pass

            # ΠΕΡΙΠΤΩΣΗ 3: Στήλες με τύπο object (συνήθως strings)
            elif self.df[col].dtype == 'object':
                try:
                    # Παίρνουμε δείγμα από τις πρώτες 5 μη-κενές τιμές
                    sample_values = self.df[col].dropna().head(5)

                    # Έλεγχος 1: Το όνομα της στήλης υποδηλώνει ημερομηνία;
                    column_suggests_datetime = any(keyword in col.lower() for keyword in
                                                   ['date', 'time', 'timestamp', 'created', 'updated', 'ημερομηνια',
                                                    'χρονος'])

                    # Ελέγχουμε αν υπάρχουν τιμές και αν είναι strings με ακριβώς 19 ψηφία
                    # Το regex r'^\d{19}$' ταιριάζει με strings που:
                    # ^ = ξεκινούν από την αρχή
                    # \d{19} = έχουν ακριβώς 19 ψηφία
                    # $ = τελειώνουν στο τέλος
                    if (len(sample_values) > 0 and
                        sample_values.astype(str).str.match(r'^\d{19}$').any() and
                        column_suggests_datetime):  # ΑΛΛΑΓΗ 1: Προστέθηκε η συνθήκη ***

                        # Μετατρέπουμε τα string timestamps σε αριθμούς
                        numeric_values = pd.to_numeric(self.df[col])

                        # Έλεγχος 2: Οι ημερομηνίες είναι σε λογικό εύρος; (1970-2100)
                        converted_dates = pd.to_datetime(numeric_values)
                        min_reasonable_date = pd.Timestamp('1970-01-01')
                        max_reasonable_date = pd.Timestamp('2100-01-01')

                        if ((converted_dates >= min_reasonable_date) &
                            (converted_dates <= max_reasonable_date)).all():
                            # Και στη συνέχεια σε datetime strings
                            self.df[col] = converted_dates.dt.strftime('%Y-%m-%d %H:%M:%S')

                except (TypeError, ValueError):
                    # Αν η μετατροπή αποτύχει, αφήνουμε τη στήλη ως έχει
                    pass
