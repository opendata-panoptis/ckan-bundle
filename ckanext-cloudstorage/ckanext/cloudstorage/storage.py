...
class ResourceCloudStorage(CloudStorage):
    def __init__(self, resource):
        ...
        if (
            ...
            upload_field_storage.filename = greek_to_greeklish(upload_field_storage.filename)
    ...
    def upload(self, id, max_size=10):
        ...
        if self.filename:
            ...
            self.filename = greek_to_greeklish(self.filename)

            # ----------------------------------------------------------------------
            # Case 1: Upload using Azure Blob Storage (new library, advanced path)
            # ----------------------------------------------------------------------
            if self.can_use_advanced_azure:
                ...
                # Upload file directly as a stream
                stream = self.file_upload
                blob_client.upload_blob(stream, overwrite=True)

                # Optionally set the correct Content-Type (MIME type)
                ...
                # ----------------------------------------------------------------------
                # Case 2: Upload using generic cloud container (libcloud-style)
                # ----------------------------------------------------------------------
                try:
                    file_upload = self.file_upload

                    # --------------------------------------------------------------
                    # Check if object already exists in the cloud
                    #   - If found, compare size and hash to avoid duplicate upload
                    # --------------------------------------------------------------
                    ...
                    try:
                        ...
                        if file_size == int(cloud_object.size):
                            ...
                            if multi_hash_file == cloud_object.hash:
                                log.debug("\t => File found, matching hash, skipping upload")
                                return
                        log.debug("\t Resource found but outdated, uploading")
                    ...
                    #   - Special handling for SpooledTemporaryFile (performance)
                    # --------------------------------------------------------------
