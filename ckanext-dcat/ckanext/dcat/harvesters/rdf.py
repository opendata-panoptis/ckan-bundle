...
class DCATRDFHarvester(DCATHarvester):
    ...
    def validate_config(self, source_config):
        ...
        if 'max_pages' in source_config_obj:
            max_pages = source_config_obj['max_pages']
    ...
    def gather_stage(self, harvest_job):
        ...
        while next_page_url:
            ...
            if max_pages and current_page > max_pages:
                log.info('Reached maximum page limit of %d pages' % max_pages)
                break
            current_page += 1
            ...
            try:
                ...
                log.error('Failed to get content from %s: %s' % (next_page_url, str(e)))
                self._save_gather_error('Failed to get content from %s: %s' % (next_page_url, str(e)), harvest_job)
                break
            ...
            # Check if we got HTML error page instead of RDF
            if content.strip().startswith('<!DOCTYPE html>') or '<title>Error' in content:
                log.error('Received HTML error page instead of RDF content from %s' % next_page_url)
                self._save_gather_error('Received HTML error page instead of RDF content from %s' % next_page_url, harvest_job)
                break
            ...
            try:
                ...
                for dataset in parser.datasets():
                    if not dataset.get('name'):
                        # Handle missing title field with fallback logic
                        title = dataset.get('title')
                        if not title and dataset.get('title_translated'):
