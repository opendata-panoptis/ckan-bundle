...
def resource_data(id, resource_id, rows=None):
    ...
    if p.toolkit.request.method == "POST":
        ...
        try:
            ...
            h.flash_success(_("Resource %s has been submitted for loading into the DataStore") % resource_id)
        except p.toolkit.ValidationError as e:
            h.flash_error(_("Error submitting resource %s to DataStore: %s") % (resource_id, str(e)))
