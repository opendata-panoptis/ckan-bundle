...
def xloader_data_into_datastore(input):
    ...
    # Ensure we have the job_id in the job_dict for the callback
    if 'job_id' not in job_dict and get_current_job():
        job_dict['job_id'] = get_current_job().id
    ...
    try:
        ...
        if job_dict.get('status') is None:
            if errored:
                job_dict['status'] = 'error'
            else:
                job_dict['status'] = 'complete'
        ...
        # Make sure we have a resource_id for logging
        resource_id = job_dict.get('metadata', {}).get('resource_id', 'unknown')
        
        log.info('Finalizing job for resource %s with status %s', 
                resource_id, job_dict.get('status', 'unknown'))
        
        # Update the task status using the callback mechanism
        callback_success = False
        if 'result_url' in input:
            try:
                callback_success = callback_xloader_hook(
                    result_url=input['result_url'],
                    api_key=input['api_key'],
                    job_dict=job_dict
                )
                if callback_success:
                    log.info('Successfully updated status for resource %s', resource_id)
            except Exception as e:
                log.error('Error updating status for resource %s: %s', resource_id, str(e))
                # Re-raise to make the error visible in logs
                raise
        else:
            log.warning('No result_url in input, cannot update status for resource %s', resource_id)
        
        # Update the errored flag based on callback success
        errored = errored or not callback_success
...
def xloader_data_into_datastore_(input, job_dict, logger):
    ...
    * calls back to CKAN with the new status
    ...
    # Log whether result_url is present in the input
    if 'result_url' in input:
        logger.info('result_url is present in input: %s', input['result_url'])
    else:
        logger.warning('result_url is NOT present in input - status updates may not work correctly')
    ...
    def direct_load():
        ...
        # Update status to running_but_viewable
        job_dict['status'] = 'running_but_viewable'
        
        # Update status using the callback mechanism
        if 'result_url' in input:
            try:
                logger.info('Updating status to running_but_viewable for resource %s', resource['id'])
                is_saved_ok = callback_xloader_hook(result_url=input['result_url'],
                                                   api_key=api_key,
                                                   job_dict=job_dict)
                if is_saved_ok:
                    logger.info('Successfully updated status to running_but_viewable for resource %s', resource['id'])
                else:
                    logger.warning('Failed to update status to running_but_viewable for resource %s', resource['id'])
            except Exception as e:
                logger.error('Error updating status to running_but_viewable: %s', str(e))
                # Continue with the load process even if status update fails
        else:
            logger.warning('No result_url in input, cannot update status for resource %s', resource['id'])
...
def callback_xloader_hook(result_url, api_key, job_dict):
    ...
    Returns whether it managed to call the callback successfully.
    
    This is a version that uses the primary callback
    and one fallback via the API action.
    '''
    log = logging.getLogger(__name__)
    status = job_dict.get('status', 'unknown')
    resource_id = job_dict.get('metadata', {}).get('resource_id', 'unknown')
    error = job_dict.get('error', None)
    
    log.info('Calling xloader_hook callback for resource %s with status: %s', resource_id, status)
    ...
    # Try the primary callback mechanism (HTTP POST to result_url)
    try:
        callback_url = modify_input_url(result_url)  # modify with local config
        ...
        if result.status_code == requests.codes.ok:
            log.info('Successfully updated status to %s for resource %s', status, resource_id)
            return True
        else:
            log.warning('Callback failed with status code %s for resource %s', 
                       result.status_code, resource_id)
    except Exception as e:
        log.warning('Error when calling xloader_hook for resource %s: %s', 
                   resource_id, str(e))
    
    # If primary callback fails, try the API action fallback
    log.info('Attempting fallback: update task_status via API for resource %s', resource_id)
    try:
        context = {'ignore_auth': True}
        task = get_action('task_status_show')(context, {
        ...
        if error:
            task['error'] = error
        get_action('task_status_update')(context, task)
        log.info('Successfully updated task_status via API for resource %s', resource_id)
        return True
    except Exception as e:
        log.error('Failed to update task_status via API: %s', str(e))
        # Re-raise the exception to make it visible in logs
        raise
