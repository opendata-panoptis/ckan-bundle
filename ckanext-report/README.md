...
* Stop a report from being generated multiple times in parallel (unnecessary waste) - use a queue?
  * **Problem**: If multiple users (or the same user with page refresh) click "Generate" almost simultaneously, the same report may start running multiple times in parallel. This wastes CPU/memory resources and can cause race conditions when writing to the same cache entry.
  * **Importance**: Prevents duplicate resource usage on the server and ensures data_cache remains consistent.
  * **Possible implementations**: Database/Redis locking - when execution begins, write "IN PROGRESS" status and have other calls wait or cancel; or use a job queue (e.g., RQ/Celery) with a unique job key (report-name+options) where the worker rejects or merges duplicate calls.

* Stop more than one report being generated in parallel (high load for the server) - maybe use a queue.
  * **Problem**: Even with different reports, if an admin clicks "Generate all" or runs a nightly cron job, many heavy queries might start simultaneously, putting excessive load on the database/Solr.
  * **Importance**: Prevents CPU/I/O spikes that could impact CKAN performance and makes nightly batch processing more predictable.
  * **Possible implementations**: Serial queue or concurrency limit - use RQ/Celery with --concurrency=1 (single worker for reports); simple solution: cron script that calls each report serially; advanced: multiple workers with rate-limiting (e.g., 2 jobs/node).
