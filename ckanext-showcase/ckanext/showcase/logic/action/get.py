...
def showcase_list(context, data_dict):
    '''Return a list of all approved showcases in the site.

    Filters showcases to display only those with approval_status='approved'
    by joining with the package_extra table.
    '''
    ...
    # Φιλτράρουμε μόνο τα showcases που έχουν approval_status='approved'
    # μέσω JOIN με τον πίνακα package_extra
    q = q.join(PackageExtra, and_(
        PackageExtra.package_id == model.Package.id,
        PackageExtra.key == 'approval_status',
        PackageExtra.value == 'approved'
    ))
...
def package_showcase_list(context, data_dict):
    ...
    if showcase_id_list:
        ...
        fq += ' AND extras_approval_status:approved'
        q = 'id:(' + ' OR '.join(['{0}'.format(x) for x in id_list]) + ')'
