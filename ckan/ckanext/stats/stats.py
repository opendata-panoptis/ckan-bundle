...
class Stats(object):
    ...
    def datasets_by_theme(self):
        """Return number of datasets per theme with both code and label"""
        try:
            current_lang = tk.h.lang()
            label_field = 'label_{}'.format(current_lang)
            ...
            for tag in vocab_data.get('tags', []):
                theme_uri = tag.get('value_uri', '')
                if not theme_uri:
                    continue

                theme_code = theme_uri.split('/')[-1]
                theme_label = tag.get(label_field)
                if tag.get(label_field) is None:
                    theme_label = theme_code

                search_result = tk.get_action('package_search')({}, {
                    'q': f'theme:*{theme_code}*',
                    'fq': 'dataset_type:dataset',
                    'rows': 0
                })

                # Επιστρέφουμε tuple με (κωδικό, ετικέτα, πλήθος)
                results.append((theme_code, theme_label, search_result['count']))

            # Φθίνουσα ταξινόμηση με βάση το πλήθος
            return sorted(results, key=lambda x: -x[2])

        except tk.ObjectNotFound:
            return []
        except Exception as e:
            log.error(f'Error in datasets_by_theme: {str(e)}')
            return []

    def organizations_by_publisher_type(self):
        """Return number of organizations per publisher type with both URI and label"""
        try:
            # Get vocabulary data for Publisher type
            vocab_data = tk.get_action('vocabularyadmin_vocabulary_show')({}, {
                'id': 'Publisher type'
            })
            ...
            for tag in vocab_data.get('tags', []):
                publisher_type_uri = tag.get('value_uri', '')
                if not publisher_type_uri:
                    continue

                # Extract the code from URI (e.g. NationalAuthority from http://purl.org/adms/publishertype/NationalAuthority)
                publisher_type_code = publisher_type_uri.split('/')[-1]
                publisher_type_label = tag.get(label_field)
                if tag.get(label_field) is None:
