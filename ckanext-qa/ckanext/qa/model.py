...
class QA(Base):
    ...
    # Metadata Quality Assessment scores
    mqa_score = Column(types.Float)
    mqa_findability_score = Column(types.Float)
    mqa_accessibility_score = Column(types.Float)
    mqa_interoperability_score = Column(types.Float)
    mqa_reusability_score = Column(types.Float)
    mqa_contextuality_score = Column(types.Float)
...
def aggregate_qa_for_a_dataset(qa_objs):
    ...
    :returns: QA dict about the dataset, with keys:
                ...
                mqa_score
                mqa_findability_score
                mqa_accessibility_score
                mqa_interoperability_score
                mqa_reusability_score
                mqa_contextuality_score
                updated
    '''
    qa_dict = {'openness_score': None, 'openness_score_reason': None,
               'mqa_score': None, 'mqa_findability_score': None,
               'mqa_accessibility_score': None, 'mqa_interoperability_score': None,
               'mqa_reusability_score': None, 'mqa_contextuality_score': None,
               'updated': None}

    # MQA scores are now stored only in the first resource's QA record
    # Get the first QA object that has MQA scores
    mqa_qa = None
    if qa_objs:
        # First try to find the first resource with MQA scores
        for qa in qa_objs:
            if qa.mqa_score is not None:
                mqa_qa = qa
                break

    # Process each QA object for openness scores
    for qa in qa_objs:
    ...
    # If we found a QA object with MQA scores, use those scores directly
    if mqa_qa:
        qa_dict['mqa_score'] = mqa_qa.mqa_score
        qa_dict['mqa_findability_score'] = mqa_qa.mqa_findability_score
        qa_dict['mqa_accessibility_score'] = mqa_qa.mqa_accessibility_score
        qa_dict['mqa_interoperability_score'] = mqa_qa.mqa_interoperability_score
        qa_dict['mqa_reusability_score'] = mqa_qa.mqa_reusability_score
        qa_dict['mqa_contextuality_score'] = mqa_qa.mqa_contextuality_score
