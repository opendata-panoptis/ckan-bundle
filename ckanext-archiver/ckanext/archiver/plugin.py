...
class ArchiverPlugin(p.SingletonPlugin, p.toolkit.DefaultDatasetForm):
    ...
    def get_blueprint(self):
        azure_bp = Blueprint('archiver', __name__)

        # Endpoint: /cache/<id>/<resource_id>/<filename>
        azure_bp.add_url_rule(
            '/cached/<id>/<resource_id>/<filename>',
            view_func=self.cached_resource_redirect,
            endpoint='cached_resource_redirect'
        )
        return azure_bp
