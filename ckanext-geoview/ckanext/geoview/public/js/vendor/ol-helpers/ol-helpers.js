...
(function() {
    ...
    OL_HELPERS.createKMLLayer = function (url) {
        var isKmz = /\.kmz(\?|$)/i.test(url);

        var kmlLayerRef; // for onread to avoid undefined reference

        var kmlFormat = new OL_HELPERS.format.KML({
            onread: function(node) {
                try {
                    if (!node || !kmlLayerRef) return;
                    var nameNode = node.querySelector(":scope > name");
                    var name = nameNode && nameNode.textContent;
                    if (name) kmlLayerRef.set('title', name);
        ...
        function handleSuccess(features, projection, source) {
            try {
        ...
        // same-origin? don't use proxy (especially in dev server)
        function isSameOrigin(u, base) {
            try { var a=new URL(u, base), b=new URL(base); return a.origin===b.origin; } catch(e){ return false; }
        }

        var effectiveUrl = url; // default: use as is (or already proxied before calling here)
        if (window.ckan && window.ckan.geoview && window.ckan.geoview.site_url) {
            if (isSameOrigin(url, window.ckan.geoview.site_url)) {
                effectiveUrl = url; // direct
            }
        }
        ...
        var source = new ol.source.Vector({
            loader: function(extent, resolution, projection) {
                var that = this;
                that.setState(ol.source.State.LOADING);

                if (isKmz) {
                    // Needs JSZip. If not available, fail gracefully.
                    if (typeof JSZip === 'undefined') {
                        handleError(new Error('KMZ not supported without JSZip'));
                        return;
                    }
                    fetch(effectiveUrl, { credentials: 'include' })
                        .then(r => { if (!r.ok) throw new Error('HTTP '+r.status); return r.arrayBuffer(); })
                        .then(buf => JSZip.loadAsync(buf))
                        .then(zip => {
                            var entry = zip.file(/\.kml$/i)[0];
                            if (!entry) throw new Error('KMZ: no .kml inside');
                            return entry.async('string');
                        })
                        .then(kmlText => {
                            var feats = kmlFormat.readFeatures(kmlText, { featureProjection: projection });
                            handleSuccess(feats, projection, that);
                        })
                        .catch(handleError);
                } else {
                    fetch(effectiveUrl, { credentials: 'include' })
        ...
        var kml = kmlLayerRef = new ol.layer.Vector({
    ...
    OL_HELPERS.withWMTSLayers = function (capaUrl, layerProcessor, layerNames, projection, resolutions, matrixSet) {
        ...
        OL_HELPERS.parseWMTSCapas(
            ...
            function (capas) {
                ...
                // Guard against missing or malformed capabilities
                if (!capas || !capas.Contents || !Array.isArray(capas.Contents.Layer) || capas.Contents.Layer.length === 0) {
                    return deferredResult.reject(new Error('WMTS capabilities contained no layers'));
                }
