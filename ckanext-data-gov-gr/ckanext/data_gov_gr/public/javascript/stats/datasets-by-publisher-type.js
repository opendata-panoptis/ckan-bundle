document.addEventListener('DOMContentLoaded', function() {
    const chartContainer = document.getElementById('publisher-type-datasets-pie-chart');
    if (!chartContainer) return;
    const chart = echarts.init(chartContainer);

    const table = document.querySelector('#stats-datasets-per-publisher-type table');
    if (!table) return;
    ...
    function updateChartLayout() {
        ...
        const option = {
            ...
            legend: {
                ...
                itemWidth: 10,
                itemHeight: 10,
                textStyle: { fontSize: 12, padding: [3, 0, 0, 0] },
                formatter: function(name) { return name.length > 40 ? name.substring(0, 37) + '...' : name; }
            },
            series: [{
                type: 'pie',
        ...
        chartContainer.style.height = isMobile ? `${Math.max(600, legendHeight + 350)}px` : '500px';
        chart.resize();
    ...
    const toggleBtn = document.getElementById('togglePublisherDatasetView');
    const tableView = document.getElementById('publisher-type-datasets-table-view');
    let showingPie = true;
