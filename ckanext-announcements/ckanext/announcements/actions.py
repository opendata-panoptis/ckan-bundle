...
def announcement_create(context, data_dict):
    ...
    formatted_from_date = from_date.strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]
    formatted_to_date = to_date.strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]
...
def announcement_update(context, data_dict):
    ...
    formatted_from_date = data_dict["from_date"].strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]
    formatted_to_date = data_dict["to_date"].strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]

    announcement.from_date = formatted_from_date
    announcement.to_date = formatted_to_date
