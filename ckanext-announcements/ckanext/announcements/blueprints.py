...
def get_dates(form_data):
    from_date = form_data.get('from_date')
    to_date = form_data.get('to_date')

    # Handle datetime format with seconds
    if from_date:
        if len(from_date) > 16:  # Format: YYYY-MM-DDTHH:MM:SS
            from_date = datetime.strptime(from_date, "%Y-%m-%dT%H:%M:%S")
        else:  # Format: YYYY-MM-DDTHH:MM
            from_date = datetime.strptime(from_date, "%Y-%m-%dT%H:%M")

    if to_date:
        if len(to_date) > 16:  # Format: YYYY-MM-DDTHH:MM:SS
